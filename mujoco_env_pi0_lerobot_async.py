"""MuJoCo Simulation using LeRobot's Official Asynchronous Inference Stack with Pi0 Policy.

Decouples Pi0 neural network inference from MuJoCo physics execution using LeRobot's
`async_inference` paradigm and Real-Time Control (RTC), eliminating waiting lags and
providing smooth action queueing with temporal action aggregation for a single environment.

Model Checkpoint : "castanetnicolas/pi0_ur5e_pick_place"
Dataset          : "castanetnicolas/UR5e_sim_pick_place_can_100"
...
# 3. Load Pi0 Policy fine-tuned on castanetnicolas/UR5e_sim_pick_place_can_100
model_id = "castanetnicolas/pi0_ur5e_pick_place"
print(f"Loading fine-tuned Pi0 policy checkpoint '{model_id}'...")
...
def format_obs_for_policy(obs_dict, task_description):
    """Format Gym environment observation dictionary into LeRobot Pi0 policy input format."""
    ...
    return {
        "observation.images.image": cam1,
        "observation.images.image2": cam2,
...
def weighted_average_aggregate(
    ...
    """Temporal action chunk aggregation function (LeRobot weighted average blending for spatial position)."""
...
def async_inference_worker():
    """Background policy server worker running Pi0 chunk inference in parallel."""
    print("LeRobot PolicyServer async Pi0 inference worker thread active.")
    current_episode = -1
...
print(f"\nStarting LeRobot Async Pi0 MuJoCo GUI Viewer (Single environment, {FPS} FPS)...")
